...
Функция ДобавитьПрисоединенныйФайл(ПараметрыФайла,
	...
	Если ПараметрыФайла.Свойство("ДопПараметры") Тогда
		ЗаполнитьЗначенияСвойств(ПрисоединенныйФайл, ПараметрыФайла.ДопПараметры);	
	КонецЕсли;
	
	Если ПараметрыФайла.Свойство("ВыгружатьВЛК") Тогда
		ПрисоединенныйФайл.ВыгружатьВЛК = ПараметрыФайла.ВыгружатьВЛК;	
	КонецЕсли;
	...
	Попытка
		...
		Иначе
			...
			// добавим УИД владельца - иначе если имена файла в одном дне одинаковые - то будут дубли
			ИмяБезРасширения_УИД = СокрЛП(ИмяБезРасширения) + "-" + НоваяСсылкаНаФайл.УникальныйИдентификатор();
			...
			СведенияОФайле = ФайловыеФункцииСлужебный.ДобавитьФайлВТом(ДвоичныеДанные, ВремяИзмененияУниверсальное,
				ИмяБезРасширения_УИД, РасширениеБезТочки, , ПрисоединенныйФайл.Зашифрован);
